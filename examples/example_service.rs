use std::{collections::BTreeMap, vec, net::{Ipv4Addr, SocketAddr, IpAddr}, time::{Duration, Instant}, sync::{Arc, Mutex}};
...
async fn main() {
    ...
    definition.methods.insert("timer".to_owned(), 
        IoTScapeMethodDescription { 
            documentation: Some("Sends timer event on a delay".to_owned()), 
            params: vec![
                IoTScapeMethodParam { name: "msec".to_owned(), documentation: Some("Amount of time to wait, in ms".to_owned()), r#type: "number".to_owned(), optional: false },
            ], 
            returns: IoTScapeMethodReturns { 
    ...
    definition.events.insert("timer".to_owned(), IoTScapeEventDescription { params: vec![] });
    ...
    loop {
        ...
        while service.lock().unwrap().rx_queue.len() > 0 {
            ...
            match next_msg.function.as_str() {
                "helloWorld" => {
                    service.lock().unwrap().enqueue_response_to(next_msg, Ok(vec!["Hello, World!".to_owned()]));
                }
                "add" => {
                    let result: f64 = next_msg.params.iter().map(|v| v.as_f64().unwrap_or_default()).sum();
                    service.lock().unwrap().enqueue_response_to(next_msg, Ok(vec![result.to_string()]));
                },
                "timer" => {
                    let ms = next_msg.params.get(0).unwrap_or(&serde_json::Value::Number(serde_json::Number::from_f64(0.0).unwrap())).as_u64().unwrap();
                    tokio::spawn(delayed_event(Arc::clone(&service), ms, next_msg.id, "timer", BTreeMap::new()));
                },
                _ => {}
            }
        }
...
async fn delayed_event(service: Arc<Mutex<IoTScapeService>>, delay: u64, call_id: String, event_type: &str, args: BTreeMap<String, String>) {
    tokio::time::sleep(Duration::from_millis(delay)).await;
    service.lock().unwrap().send_event(call_id.as_str(), event_type, args);
}